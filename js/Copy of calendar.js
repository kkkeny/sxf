...
function tiannetPopCalendar() {
    ...
    tiannetWriteHead();
    showTime();
    var dads = $('#divTiannetDate');
    var input = $(tiannetOutObject), pos = input.offset();
    dads.css('left', pos.left);
    dads.css('top', pos.top + input.height() + 5);
    tiannetShowControl();
    selectObject();
}
...
function tiannetHideObject(strTagName) {
    ...
    var getTags = function(tagname) {
        return document.getElementsByTagName(tagname);
    };
    for (var i = 0; i < getTags(strTagName).length; i++) {

        var obj = getTags(strTagName)[i];
